...
private val logger: Logger = LoggerFactory.getLogger("OutboundMapper")

fun Content.toOutbound() = ContentDto(
    id = extractExternalId(id, teamOwnedBy),
    href = href,
    title = languageSubfieldValue(title, language) ?: handleMissingValue(id, TITLE),
    ingress = languageSubfieldValue(ingress, language) ?: handleMissingValue(id, INGRESS),
    text = languageSubfieldValue(text, language) ?: handleMissingValue(id, TEXT),
